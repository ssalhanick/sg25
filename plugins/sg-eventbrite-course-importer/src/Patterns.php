 ...
 * Class Patterns.
 *
 * Handles block pattern registration and management for the plugin.
 */
class Patterns {
	...
	public function __construct() {
		// Initialize patterns if WordPress is ready
		if ( did_action( 'init' ) ) {
			$this->init_custom_patterns();
		} else {
			add_action( 'init', array( $this, 'init_custom_patterns' ) );
		}
	}
	 ...
	 */
	public function init_custom_patterns() {
	...
	public function register_pattern_categories() {
		register_block_pattern_category(
			'sg-eventbrite-course-importer-patterns',
			array(
				'label' => __( 'Eventbrite Course Importer Patterns', 'sg-eventbrite-course-importer' ),
			)
		);
	}
	...
	private function load_pattern_files() {
		$patterns_path = SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_PATH . '/patterns';
	...
	private function define_default_patterns() {
		...
		$this->custom_patterns['sg-eventbrite-course-importer/hero-section'] = array(
			'title'       => __( 'Eventbrite Course Importer Hero Section', 'sg-eventbrite-course-importer' ),
			'description' => __( 'A hero section with heading, text, and call-to-action button.', 'sg-eventbrite-course-importer' ),
			'categories'  => array( 'header', 'hero', 'sg-eventbrite-course-importer-patterns' ),
...
<div class="wp-block-group alignfull has-white-color has-primary-background-color has-text-color has-background" style="padding-top:4rem;padding-bottom:4rem">
	...
	<h1 class="wp-block-heading has-text-align-center has-x-large-font-size">Welcome to Eventbrite Course Importer</h1>
	<!-- /wp:heading -->
	
	<!-- wp:paragraph {"align":"center","fontSize":"medium"} -->
	<p class="has-text-align-center has-medium-font-size">This is a hero section created with the Eventbrite Course Importer plugin.</p>
	<!-- /wp:paragraph -->
...
<!-- /wp:group -->',
		...
		$this->custom_patterns['sg-eventbrite-course-importer/two-column-content'] = array(
			'title'       => __( 'Eventbrite Course Importer Two Column Content', 'sg-eventbrite-course-importer' ),
			'description' => __( 'Two column layout with image and text content.', 'sg-eventbrite-course-importer' ),
			'categories'  => array( 'columns', 'text', 'sg-eventbrite-course-importer-patterns' ),
...
<!-- /wp:columns -->',
		...
		$this->custom_patterns['sg-eventbrite-course-importer/call-to-action'] = array(
			'title'       => __( 'Eventbrite Course Importer Call to Action', 'sg-eventbrite-course-importer' ),
			'description' => __( 'A call-to-action section with centered content.', 'sg-eventbrite-course-importer' ),
			'categories'  => array( 'call-to-action', 'buttons', 'sg-eventbrite-course-importer-patterns' ),
...
<div class="wp-block-group alignfull has-light-gray-background-color has-background" style="padding-top:3rem;padding-bottom:3rem">
	...
	<p class="has-text-align-center">Take action today and discover what Eventbrite Course Importer can do for you.</p>
	<!-- /wp:paragraph -->
...
<!-- /wp:group -->',
	...
	private function register_pattern( $pattern_name, $pattern_data ) {
		...
		$pattern_data = wp_parse_args( $pattern_data, array(
			'description' => '',
			'categories'  => array( 'sg-eventbrite-course-importer-patterns' ),
			'keywords'    => array(),
			'viewport'    => 800,
		));
