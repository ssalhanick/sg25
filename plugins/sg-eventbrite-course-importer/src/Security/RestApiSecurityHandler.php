...
class RestApiSecurityHandler {
	...
	public static function register_endpoint( $namespace_api, $route, $args = array(), $security_config = array() ) {
		...
		add_action( 'rest_api_init', function() use ( $namespace_api, $route, $args ) {
	...
	private static function handle_secure_request( $request, $callback, $config, $endpoint_key ) {
		try {
			...
			error_log( '[sg-eventbrite-course-importer] REST API Security Error: ' . $e->getMessage() );
			return new \WP_Error(
				'security_error',
				$e->getMessage(),
	 ...
	 */
	public static function localize_rest_data( $handle, $object_name = 'sgEventbriteCourseImporterRestApi', $additional_data = array() ) {
		$rest_data = wp_parse_args(
			$additional_data,
			array(
				'restUrl'   => rest_url(),
				'nonce'     => self::create_nonce(),
				'namespace' => 'sg-eventbrite-course-importer/v1',
			)
		);
