...
namespace SG\EventbriteCourseImporter\Security;
...
class AjaxSecurityHandler {
	...
	public static function handle_ajax_request() {
		...
		try {
			...
			error_log( '[sg-eventbrite-course-importer] AJAX Security Error: ' . $e->getMessage() );
			}
	...
	private static function perform_security_checks( $action, $config ) {
		...
		if ( $config['nonce_required'] ) {
			...
			$nonce_action = 'sg-eventbrite-course-importer_' . $action;
	...
	private static function validate_request_data( $validation_rules ) {
		...
		$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'sg-eventbrite-course-importer_ajax' ) ) {
			throw new \Exception( 'Invalid security token' );
		}
	...
	public static function create_nonce( $action ) {
		return wp_create_nonce( 'sg-eventbrite-course-importer_' . $action );
	}
	...
	public static function verify_nonce( $nonce, $action ) {
		return wp_verify_nonce( $nonce, 'sg-eventbrite-course-importer_' . $action );
	}
} 