# Eventbrite Course Importer

This plugin imports eventbrite events as courses, using the Eventbrite API key.
...
1. Clone this repository to your plugins directory:
   ...
   git clone [repository-url] sg-eventbrite-course-importer
   ```

2. Install dependencies:
   ```bash
   cd sg-eventbrite-course-importer
   composer install
   npm install
   ```
...
```php
<?php
use SG\EventbriteCourseImporter\Plugin;
...
```php
<?php
use SG\EventbriteCourseImporter\Assets;
...
```php
<?php
use SG\EventbriteCourseImporter\Security\SecurityValidator;
...
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_FILE` - Main plugin file path
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_PATH` - Plugin directory path
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_URL` - Plugin directory URL
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_BUILD_PATH` - Built assets directory path
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_BUILD_URL` - Built assets directory URL
- `SG_EVENTBRITE_COURSE_IMPORTER_PLUGIN_VERSION` - Plugin version

## JavaScript Global

The plugin exposes a global JavaScript object `sgEventbriteCourseImporter` with:

```javascript
window.sgEventbriteCourseImporter = {
    ajax_url: '/wp-admin/admin-ajax.php',
    nonce: 'security-nonce',
    plugin_url: '/wp-content/plugins/sg-eventbrite-course-importer',
    version: '1.0.0'
};
```
