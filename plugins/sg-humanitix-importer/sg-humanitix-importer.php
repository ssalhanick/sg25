 ...
 * Main plugin File
 *
 * @package Utd\SG\HumanitixImporter
 *
 * Plugin Name:       Humanitix Importer
 * Plugin URI:        https://webscvcs.utdallas.edu
 * Description:       HUmanitix API importer for THe Events Calendar
 * Version:           1.0.0
 * Requires at least: 6.5
 * Requires PHP:      8.0+
 * Author:            Scott Salhanick
 * Author URI:        https://webscvcs.utdallas.edu
...
add_action(
	'plugins_loaded',
	function () {
		if ( ! class_exists( 'Utd\\SG\HumanitixImporter\\Plugin' ) ) {
			return;
		}

		$plugin = new Utd\SG\HumanitixImporter\Plugin();
...
add_action(
	...
	function () {
		if ( ! is_admin() ) {
			...
			class_exists( 'Utd\\SG\HumanitixImporter\\Assets' );
