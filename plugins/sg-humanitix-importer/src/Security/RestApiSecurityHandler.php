...
class RestApiSecurityHandler {
	 ...
	 */
	public static function register_endpoint( $namespace, $route, $args = array(), $security_config = array() ) {
		$defaults = array(
	...
	private static function handle_secure_request( $request, $callback, $config, $endpoint_key ) {
		try {
			...
			error_log( '[sg-humanitix-importer] REST API Security Error: ' . $e->getMessage() );
			return new \WP_Error(
				'security_error',
				$e->getMessage(),
				array( 'status' => $e->getCode() ?: 403 )
			);
		}
	}
	...
	private static function check_rate_limit( $endpoint_key, $max_requests_per_minute ) {
		...
		$key = $endpoint_key . '_' . ( $user_id ?: $ip_address );
		$current_time = time();
	 ...
	 */
	public static function get_rest_url( $namespace = '', $route = '' ) {
		if ( $namespace && $route ) {
	 ...
	 */
	public static function localize_rest_data( $handle, $object_name = 'sgHumanitixImporterRestApi', $additional_data = array() ) {
		$rest_data = wp_parse_args(
			$additional_data,
			array(
				'restUrl'   => rest_url(),
				'nonce'     => self::create_nonce(),
				'namespace' => 'sg-humanitix-importer/v1',
			)
		);
