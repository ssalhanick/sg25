...
document.addEventListener('DOMContentLoaded', function () {
	...
	console.log(
		'humanitixAdmin object:',
		typeof humanitixAdmin !== 'undefined' ? humanitixAdmin : 'NOT DEFINED'
	);
	...
	if (testApiButton) {
		testApiButton.addEventListener('click', function (e) {
			...
			resultDiv.innerHTML =
				'<div class="notice notice-info"><p>Testing API connection...</p></div>';
			...
			fetch(humanitixAdmin.ajaxUrl, {
				...
				.catch((error) => {
					resultDiv.innerHTML =
						'<div class="notice notice-error"><p>Failed to test API connection. Please try again.</p></div>';
	...
	if (startImportButton) {
		startImportButton.addEventListener('click', function (e) {
			...
			statusDiv.innerHTML =
				'<span class="spinner is-active"></span> Starting import...';
			...
			fetch(humanitixAdmin.ajaxUrl, {
				...
				.then((data) => {
					if (data.success) {
						statusDiv.innerHTML =
							'<span class="dashicons dashicons-yes-alt"></span> Import completed successfully';
						const resultsContent =
							resultsDiv.querySelector('#results-content');
						resultsContent.innerHTML =
							'<p><strong>Events imported:</strong> ' +
						...
						statusDiv.innerHTML =
							'<span class="dashicons dashicons-no-alt"></span> Import failed: ' +
							data.data.message;
					}
				})
				.catch((error) => {
					statusDiv.innerHTML =
						'<span class="dashicons dashicons-no-alt"></span> Import failed. Please try again.';
	...
	if (exportLogsButton) {
		exportLogsButton.addEventListener('click', function (e) {
			...
			const url =
				humanitixAdmin.ajaxUrl +
				'?action=export_logs&level=' +
				encodeURIComponent(levelSelect.value) +
				'&date=' +
				encodeURIComponent(dateInput.value) +
				'&nonce=' +
				humanitixAdmin.logsNonce;
